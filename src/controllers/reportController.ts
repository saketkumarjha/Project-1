...
// ): Promise<void> => {
//   try {
//     const { startDate: start, endDate: end, department } = req.query;
...
//     const analytics = await AnalyticsService.getComprehensiveAnalytics(filters);
...
//     res.status(500).json({
//       success: false,
//       message: "Error fetching comprehensive analytics",
//       error: error instanceof Error ? error.message : "Unknown error",
//     });
//   }
// };

// Get KPI (Key Performance Indicators) summary
...
//     // Patient satisfaction (mock data - would come from surveys)
//     const patientSatisfaction = 4.2;

//     // Average wait time (mock calculation)
//     const averageWaitTime = 45; // minutes
...
//         $group: {
//           _id: null,
//           totalCapacity: { $sum: "$maxOccupancy" },
...
//     const occupancyRate =
//       totalBeds.length > 0
//         ? (totalBeds[0].currentOccupancy / totalBeds[0].totalCapacity) * 100
//         : 0;

//     // Staff utilization (mock data)
//     const staffUtilization = 78;
...
//         revenuePerPatient: Math.round(revenuePerPatient * 100) / 100,
//         growthRate: Math.round(growthRate * 100) / 100,
//       },
//       quality: {
//         patientSatisfaction,
//         readmissionRate: 5.2, // Mock data
//         treatmentSuccessRate: 94.8, // Mock data
...
// ): Promise<void> => {
//   try {
//     const { metric = "patients", period = "month", department } = req.query;
//     const { startDate, endDate } = getDateRange(period as string);
...
//     switch (metric) {
//       case "patients":
//         trendData = await Patient.aggregate([
//           {
//             $match: {
//               createdAt: { $gte: startDate, $lt: endDate },
//               ...departmentFilter,
...
//       case "appointments":
//         trendData = await Appointment.aggregate([
//           {
//             $match: {
//               appointmentDate: { $gte: startDate, $lt: endDate },
//               ...departmentFilter,
...
//       case "revenue":
//         trendData = await Bill.aggregate([
//           {
//             $match: {
//               createdAt: { $gte: startDate, $lt: endDate },
//               status: { $in: ["paid", "partially_paid"] },
...
//           success: false,
//           message:
//             "Invalid metric. Use 'patients', 'appointments', or 'revenue'",
//         });
//         return;
//     }
...
//     const firstValue =
//       trendData.length > 0 ? trendData[0].count || trendData[0].amount : 0;
//     const lastValue =
//       trendData.length > 0
