...
/**
 * Core Express Middleware Setup (NON-SESSION MIDDLEWARE ONLY)
 * Order matters: CORS must be before other middleware that might send responses
 * Note: Session middleware is initialized separately in initializeSessions()
 */
...
const startServer = async () => {
  try {
    ...
    // ✅ CRITICAL: This must happen BEFORE routes are loaded
    ...
    // Step 3: Now load routes AFTER session middleware is configured
    console.log("\n📚 Loading API routes...");
    ...
    app.get("/", (req, res) => {
      res.json({
        ...
        status: "running",
        sessionStore: sessionInitialized ? "Redis" : "Memory",
     ...
     * Use this to verify session persistence across requests
     */
      ...
      try {
        ...
        res.json({
          ...
          data: {
            ...
            session: {
              store: sessionInitialized ? "redis" : "memory",
    ...
    app.listen(PORT, () => {
      ...
      console.log(
        `  ├─ Test session: http://localhost:${PORT}/api/test-session`
      );
      console.log(
